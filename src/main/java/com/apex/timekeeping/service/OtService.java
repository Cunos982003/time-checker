...
public class OtService {
    ...
    public OtResponse create(Long userId, OtRequest req) {
        ...
        return toResponse(otRepository.save(ot));
    }
    ...
    public Page<OtResponse> getPendingForManager(Long managerId, Pageable pageable) {
        return otRepository.findPendingForManager(managerId, pageable).map(this::toResponse);
    }
    ...
    public OtResponse approve(Long otId, Long approverId) {
        ...
        ot.setCfDate(java.time.LocalDate.now());
    ...
    public void convertToLeave(Long otId) {
        ...
        // 8 OT hours = 1 compensatory leave day
    ...
    private OtResponse toResponse(Ot ot) {
        return OtResponse.builder()
                .otId(ot.getOtId())
                .otDate(ot.getOtDate())
                .startTime(ot.getStartTime())
                .endTime(ot.getEndTime())
                .workHoursOt(ot.getWorkHoursOt())
                .reason(ot.getReason())
                .status(ot.getStatus().name())
                .typeName(ot.getOtType().getTypeName())
                .otRate(ot.getOtType().getOtRate())
                .projectName(ot.getProject() != null ? ot.getProject().getProjectName() : null)
                .confirmedByName(ot.getConfirmedBy() != null ? ot.getConfirmedBy().getFullname() : null)
                .cfDate(ot.getCfDate())
                .build();
    }
}
